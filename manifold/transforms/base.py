...
class CompositeTransform(Transform):
    ...
    @staticmethod
    def _cascade(device, inputs, funcs, context, full_jacobian=False):
    ...
    def forward(self, inputs, context=None, full_jacobian=False):
        ...
        return self._cascade(self.device, inputs, funcs, context, full_jacobian)
...
class MultiscaleCompositeTransform(Transform):
    ...
    def __init__(self, device, num_transforms, split_dim=1):
        """Constructor.
    ...
    def forward(self, inputs, context=None, full_jacobian=False):
        ...
        if full_jacobian:
            ...
            all_outputs = []
            total_logabsdet = torch.zeros(batch_size, device=self.device)

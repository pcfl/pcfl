...
from manifold.transforms import Transform
from manifold.various import product


class ProjectionSplit(Transform):
    ...
    def inverse(self, inputs, **kwargs):
        orthogonal_inputs = kwargs.get("orthogonal_inputs", torch.zeros(inputs.size(0), self.input_dim_total - self.output_dim, device=self.device))
        if self.mode_in == "vector" and self.mode_out == "vector":

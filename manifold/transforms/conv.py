...
class OneByOneConvolution(transforms.LULinear):
    ...
    def __init__(self, num_channels, device, using_cache=True, identity_init=True):
        super().__init__(num_channels, using_cache, identity_init)
        self.permutation = transforms.RandomPermutation(features=num_channels, device=device, dim=1)

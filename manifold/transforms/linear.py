...
class Linear(transforms.Transform):
    ...
    def forward(self, inputs, context=None, full_jacobian=False):
        if not self.training and self.using_cache:
            ...
            if full_jacobian:
                ...
                logabsdet = self.cache.logabsdet * torch.ones(outputs.shape[0], device=inputs.device)
    ...
    def inverse(self, inputs, context=None, full_jacobian=False):
        if not self.training and self.using_cache:
            ...
            if full_jacobian:
                jacobian = self.cache.inverse.repeat(outputs.shape[0], 1, 1)
                return outputs, jacobian
            else:
                logabsdet = (-self.cache.logabsdet) * torch.ones(outputs.shape[0], device=inputs.device)
                return outputs, logabsdet
...
class LULinear(Linear):
    ...
    def __init__(self, features, using_cache=True, identity_init=True, eps=1e-3):
        super().__init__(features, using_cache)
    ...
    def weight_inverse(self):
        ...
        """
        lower, upper = self._create_lower_upper()
        identity = torch.eye(self.features, self.features, device=lower.device)

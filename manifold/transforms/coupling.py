...
from manifold.transforms import splines
...
class CouplingTransform(transforms.Transform):
    ...
    def __init__(self, device, mask, transform_net_create_fn, unconditional_transform=None):
        """
        Constructor.
    ...
    def inverse(self, inputs, context=None, full_jacobian=False):
        ...
        if full_jacobian:
            ...
            # Put together full Jacobian
            batchsize = inputs.size(0)
            jacobian = torch.zeros((batchsize,) + inputs.size()[1:] + inputs.size()[1:]).to(self.device)
            jacobian[:, self.identity_features, self.identity_features] = 1.0
            jacobian[:, self.transform_features, :] = jacobian_transform

            outputs = torch.empty_like(inputs).to(self.device)
...
class PiecewiseRationalQuadraticCouplingTransform(PiecewiseCouplingTransform):
    def __init__(
        ...
        super().__init__(device, mask, transform_net_create_fn, unconditional_transform=unconditional_transform)
